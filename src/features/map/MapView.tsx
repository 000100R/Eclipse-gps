...
export const MapView: React.FC = () => {
  ...
  return (
    <div id="eclipse-map-viewport" className="absolute inset-0 w-full h-full z-0 overflow-hidden bg-neutral-950">
      ...
      {/* 🗺️ MAP HUD CONTROLS ROW (Map Type Switcher + Intelligence Grid HUD + Provider Selector - Active when NOT navigating) */}
      {!isNavigating && (
        <div
          id="map-controls-toolbar-row"
          style={{ top: 'calc(env(safe-area-inset-top, 0px) + 4.25rem)' }}
          className="absolute left-3 right-3 sm:left-4 sm:right-4 z-20 flex items-center justify-between pointer-events-none gap-1.5 sm:gap-2 max-w-lg md:max-w-none md:mx-auto"
        >
          {/* Left Cluster: Clearly visible, easy-to-tap map control button + Intelligence Grid HUD */}
          <div id="map-left-hud-controls" className="flex items-center space-x-1.5 sm:space-x-2 pointer-events-auto min-w-0">
            <div id="map-style-selector-hud" className="relative flex flex-col items-start">
              <button
                id="btn-toggle-map-style"
                onClick={() => setIsMenuOpen(!isMenuOpen)}
                className="flex items-center space-x-1 sm:space-x-2 px-2.5 sm:px-4 py-2 bg-neutral-950/90 hover:bg-neutral-900 border border-neutral-800/80 text-white font-bold text-[11px] sm:text-xs rounded-2xl shadow-2xl transition-all duration-300 select-none cursor-pointer h-11 shrink-0"
              ...
              >
                ...
                <span className="tracking-wider uppercase whitespace-nowrap">
                  <span className="hidden sm:inline">MAP: </span>{mapStyle.toUpperCase()}
                </span>
                <Layers size={12} className="text-indigo-400 ml-0.5 sm:ml-1 shrink-0" />
              </button>
              ...
              {isMenuOpen && (
                <>
                  ...
                  />
                  <GlassPanel className="absolute top-12 left-0 p-2 w-44 sm:w-48 flex flex-col space-y-1 border border-neutral-800/90 shadow-2xl rounded-2xl animate-in fade-in slide-in-from-top-2 duration-200 z-50">
          ...
          {/* Right Cluster: Map Provider Selector HUD */}
          <div id="map-provider-dock" className="pointer-events-auto shrink-0">
            <GlassPanel className="p-1 sm:p-1.5 flex items-center space-x-1 sm:space-x-1.5 border border-neutral-800/80 shadow-2xl rounded-2xl">
              <button
                id="btn-provider-leaflet"
                onClick={() => setMapProvider('leaflet')}
                className={`flex items-center space-x-1 px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-300 ${
                  mapProvider === 'leaflet'

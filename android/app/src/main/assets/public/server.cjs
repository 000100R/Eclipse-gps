...
try {
  if (import_fs.default.existsSync("/app/.dev.env.json")) {
    const devEnv = JSON.parse(import_fs.default.readFileSync("/app/.dev.env.json", "utf8"));
...
var CANDIDATE_MODELS = Array.from(
  /* @__PURE__ */ new Set([
...
function markModelCoolingDown(model, reason, seconds = 45) {
  modelCooldowns.set(model, Date.now() + seconds * 1e3);
...
function classifyGeminiError(error) {
  ...
  const sanitizedMsg = rawMsg.replace(/key=[A-Za-z0-9_-]+/gi, "key=[REDACTED]").replace(/AIza[0-9A-Za-z_-]{30,}/g, "[REDACTED_KEY]");
  return {
...
app.get("/api/ai/health", async (req, res) => {
  ...
  return res.json({
    ...
    result: responseText.includes("ECLIPSE_GEMINI_OK") ? "Gemini connection verified and active." : `Gemini responded: "${responseText}"`
  });
});
...
`;
function generateFallbackIntent(promptText, userLocation) {
  const p = (promptText || "").toLowerCase();
  if (p.includes("which metro exit") || p.includes("which metro gate") || p.includes("what metro exit") || p.includes("what metro gate") || p.includes("metro exit should i take") || p.includes("metro gate should i take") || p.includes("best metro exit") || p.includes("best metro gate") || p.includes("exit should i take for this pandal") || p.includes("metro exit for this pandal") || p.includes("metro gate for this pandal") || p.includes("metro exit") && (p.includes("pandal") || p.includes("take") || p.includes("for")) || p.includes("metro gate") && (p.includes("pandal") || p.includes("take") || p.includes("for")) || p.trim() === "which metro exit should i take for this pandal?" || p.trim() === "which metro exit should i take for this pandal") {
    return {
  ...
  if (p.includes("plan my puja route") || p.includes("plan puja route") || p.includes("plan my route") || p.includes("multi pandal route") || p.includes("multi-pandal route") || p.includes("pandal route planner") || p.includes("plan route") || p.includes("puja route") || p.trim() === "plan my puja route") {
    return {
  ...
  if (p.includes("show unvisited pandals") || p.includes("unvisited pandals") || p.includes("pandals i haven't visited") || p.includes("pandals i havent visited") || p.includes("not visited yet") || p.includes("unvisited puja") || p.includes("pujas not visited") || p.includes("unvisited") || p.trim() === "show unvisited pandals") {
    return {
      text: "Finding verified Durga Puja pandals you haven't visited yet near your GPS coordinates.",
      action: "SHOW_UNVISITED_PANDALS",
      parameters: { radius: 5e3, category: "pandal" }
    };
  }
  if (p.includes("take me to the nearest pandal") || p.includes("take me to nearest pandal") || p.includes("navigate to the nearest pandal") || p.includes("navigate to nearest pandal") || p.includes("route me to the nearest pandal") || p.includes("route me to nearest pandal") || p.includes("take me to the closest pandal") || p.includes("take me to closest pandal") || p.includes("nearest pandal") && (p.includes("take me") || p.includes("navigate") || p.includes("route") || p.includes("go to") || p.includes("lead me") || p.includes("walk") || p.includes("drive")) || p.includes("closest pandal") && (p.includes("take me") || p.includes("navigate") || p.includes("route") || p.includes("go to") || p.includes("lead me") || p.includes("walk") || p.includes("drive")) || p.trim() === "take me to the nearest pandal") {
    return {
  ...
  if (p.includes("show nearby pandals") || p.includes("nearby pandals") || p.includes("pandals nearby") || p.includes("find nearby pandals") || p.includes("pandals near me") || p.includes("nearby puja") || p.includes("pujas near me") || p.trim() === "show nearby pandals") {
    return {
  ...
  if (p.includes("best for tonight") || p.includes("pandals for tonight") || p.includes("tonight") && p.includes("pandal")) {
    return {
      ...
      action: "SEARCH_NEARBY_PANDALS",
      parameters: { radius: 6e3, category: "pandal", query: promptText }
    };
  }
  ...
  if (p.includes("metro")) {
    ...
    return {
      text: matchedStation ? `Here are verified Durga Puja pandals and heritage Bonedi Baris accessible from ${matchedStation} Metro station, with walking routes and nearest gates.` : "Here is the Eclipse Metro Puja Guide showing Kolkata Metro stations connected to nearby Durga Puja pandals and heritage Bonedi Baris.",
      action: "SEARCH_METRO",
      parameters: { query: promptText, stationName: matchedStation || void 0 }
    };
  }
  if (p.includes("bonedi") || p.includes("rajbari") || p.includes("aristocrat") || p.includes("heritage house") || p.includes("sovabazar") || p.includes("sabarna") || p.includes("chhatu babu") || p.includes("khelat ghosh") || p.includes("jorasanko") || p.includes("zamindar")) {
    const isTour = p.includes("tour") || p.includes("plan") || p.includes("itinerary") || p.includes("route");
    return {
      text: isTour ? "I've planned a verified Bonedi Bari Heritage Tour covering Kolkata's historic aristocratic houses (Sovabazar Rajbari, Chhatu Babu & Latu Babu, Pathuriaghata Khelat Ghosh, and Jorasanko Shib Krishna Daw). Loading itinerary on your map!" : "Here are Kolkata's verified Bonedi Bari heritage houses. You can examine their founding centuries, nearest metro stations, and navigate directly on the map!",
  ...
  if (p.includes("plan") || p.includes("route") || p.includes("tour") || p.includes("itinerary") || p.includes("hop") || p.includes("hours") || p.includes("walking") || p.includes("crowded stop") || p.includes("finish before")) {
    ...
    let checkFinishBeforeTime = void 0;
...
var placesSearchCache = /* @__PURE__ */ new Map();
var placesDetailsCache = /* @__PURE__ */ new Map();
var PLACES_CACHE_TTL_MS = 15 * 60 * 1e3;
...
function setPlacesCache(key, places, nextPageToken) {
...
app.post("/api/places/search", async (req, res) => {
  ...
  const locKey = location ? `${location.lat.toFixed(3)}_${location.lng.toFixed(3)}_${radius || 5e3}` : "noloc";
    ...
    if (bounds && typeof bounds.north === "number" && typeof bounds.south === "number") {
      ...
      const circleObj = {
        circle: {
          ...
          radius: Math.min(2e4, Math.max(500, radius || 3e3))
        }
      };
...
app.post("/api/places/nearby", async (req, res) => {
  ...
  const types = Array.isArray(includedTypes) && includedTypes.length > 0 ? includedTypes : ["place_of_worship", "tourist_attraction", "cultural_landmark", "community_center", "event_venue"];
  const cacheKey = `nearby_${location.lat.toFixed(3)}_${location.lng.toFixed(3)}_${radius || 2e3}_${types.join(",")}`;
  const cached = getPlacesCache(cacheKey);
  if (cached) {
    ...
    const requestBody = {
      ...
      locationRestriction: {
        circle: {
          ...
          radius: Math.min(2e4, Math.max(500, radius || 2e3))
        }
      },
      maxResultCount: 20
    };
...
app.post("/api/groups/:id/telemetry", (req, res) => {
  ...
  member.sharingEnabled = sharingEnabled !== void 0 ? sharingEnabled : member.sharingEnabled;
  if (member.sharingEnabled) {
    if (latitude !== void 0) member.latitude = latitude;
    if (longitude !== void 0) member.longitude = longitude;
    if (heading !== void 0) member.heading = heading;
    if (speed !== void 0) member.speed = speed;
    if (accuracy !== void 0) member.accuracy = accuracy;
